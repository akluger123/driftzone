...
import p5 from "p5";
import type { GameProfile } from "@/hooks/useAuth";
...
interface AuthProp {
  ...
  saveProgress: (progress: Partial<GameProfile>) => Promise<void>;
  fetchProfile: () => void;
}
...
const DriftGame = ({ auth }: DriftGameProps) => {
  ...
  useEffect(() => {
    ...
    const sketch = (p: p5) => {
      ...
      const CARS: Record<string, CarData> = {
        "Lambo Huracan": { color: [255, 165, 0], speed: 0.18, drift: 0.82, drawFn: drawLambo },
        "Nissan GT-R": { color: [180, 180, 190], speed: 0.15, drift: 0.80, drawFn: drawGTR },
        "Ford Mustang": { color: [0, 60, 180], speed: 0.16, drift: 0.85, drawFn: drawMustang },
        "Red Pro": { color: [200, 30, 30], speed: 0.13, drift: 0.88, drawFn: (p, _s, d) => drawGenericCar(p, [200, 30, 30], 1, d) },
        "Blue Turbo": { color: [30, 100, 200], speed: 0.15, drift: 0.84, drawFn: (p, _s, d) => drawGenericCar(p, [30, 100, 200], 1, d) },
        "Yellow Speed": { color: [255, 200, 0], speed: 0.18, drift: 0.86, drawFn: (p, _s, d) => drawGenericCar(p, [255, 200, 0], 1, d) },
      };

      interface DecalInfo { name: string; cost: number; id: string; }
      ...
      interface LevelDef { name: string; width: number; height: number; trackWidth: number; obstacles: number; targetTime: number; type: "track" | "openWorld" | "twisty"; }
      ...
      // Load profile data into game state
      const syncFromProfile = () => {
        const prof = authRef.current.profile;
        if (prof && !profileLoaded) {
          totalCoins = prof.total_coins;
          ownedDecals = prof.owned_decals || [];
          equippedDecal = prof.equipped_decal;
          currentCar = prof.selected_car || "Lambo Huracan";
          profileLoaded = true;
        }
      };

      // Save progress to DB (debounced in draw loop)
      const saveToCloud = () => {
        if (!authRef.current.user) return;
        authRef.current.saveProgress({
          total_coins: totalCoins,
          owned_decals: ownedDecals,
          equipped_decal: equippedDecal,
          selected_car: currentCar,
          high_score: Math.max(score, authRef.current.profile?.high_score || 0),
        });
      };
      ...
      p.draw = () => {
        ...
        if (authRef.current.profile && !profileLoaded) {
          syncFromProfile();
        }

        // Auto-save every 5 seconds if logged in
      ...
      const drawMenu = () => {
        ...
        p.textSize(56);
        p.textStyle(p.BOLD);
        p.text("DRIFT GAME - AMI", CW / 2, CH / 2 - 160);

        p.textSize(20);
        p.fill(0, 200, 0);
        p.textStyle(p.NORMAL);
        p.text("Advanced Edition", CW / 2, CH / 2 - 100);

        p.textSize(14);
        p.fill(150);
        p.text(`Coins: ${totalCoins}`, CW / 2, CH / 2 - 70);

        // Auth status
        const currentAuth = authRef.current;
        if (currentAuth.user) {
          const name = currentAuth.profile?.display_name || currentAuth.user.email || "Player";
          p.fill(0, 200, 255);
          p.textSize(13);
          p.text(`Logged in as ${name}`, CW / 2, CH / 2 - 50);
          
          // Sign out button (small, top right)
          smallBtn(CW - 100, 10, 90, 28, "Sign Out", () => {
            handleSignOut();
            profileLoaded = false;
          });
        } else {
          // Google sign in button
          btn(CW / 2 - 110, CH / 2 - 45, 220, 40, "🔑 Sign in with Google", () => {
            handleSignIn();
          });
        }

        const menuY = currentAuth.user ? CH / 2 + 10 : CH / 2 + 10;
        btn(CW / 2 - 100, menuY, 200, 50, "PLAY", () => { gameState = GAME_STATE.LEVEL_SELECT; });
        btn(CW / 2 - 100, menuY + 65, 200, 50, "GARAGE", () => { gameState = GAME_STATE.GARAGE; });
        btn(CW / 2 - 100, menuY + 130, 200, 50, "DECAL SHOP", () => { gameState = GAME_STATE.DECAL_SHOP; });
      };
      ...
      const drawGarage = () => {
        ...
        btn(CW / 2 - 80, CH - 65, 160, 45, "BACK", () => { gameState = GAME_STATE.MENU; saveToCloud(); });
      };
      ...
      const drawDecalShop = () => {
        ...
        btn(CW / 2 - 80, CH - 60, 160, 45, "BACK", () => { gameState = GAME_STATE.MENU; saveToCloud(); });
      };
      ...
      const generateOpenWorld = (level: LevelDef) => {
        ...
        for (let cluster = 0; cluster < 3; cluster++) {
          ...
          for (let i = 0; i < count; i++) {
            ...
            cones.push({ x: cx + Math.cos(angle) * radius, y: cy + Math.sin(angle) * radius });
          }
        }
        for (let i = 0; i < 8; i++) {
          cones.push({ x: p.random(200, level.width - 200), y: p.random(200, level.height - 200) });
        }
        for (let i = 0; i < 25; i++) {
          coins.push({ x: p.random(150, level.width - 150), y: p.random(150, level.height - 150), collected: false, bobOffset: p.random(p.TWO_PI) });
        }
      };
      ...
      const generateTwistyTrack = (level: LevelDef) => {
        ...
        for (let i = 0; i < numPoints; i++) {
          ...
          points.push(p.createVector(cx + Math.cos(angle) * r, cy + Math.sin(angle) * r));
        }
        ...
        for (let i = 0; i < twistyTrackPoints.length; i += 12) {
          const pt = twistyTrackPoints[i];
          coins.push({ x: pt.x + p.random(-30, 30), y: pt.y + p.random(-30, 30), collected: false, bobOffset: p.random(p.TWO_PI) });
        }
      };
      ...
      const generateObstacles = (count: number, w: number, h: number) => {
        for (let i = 0; i < count; i++) {
          ...
          do { x = p.random(100, w - 100); y = p.random(100, h - 100); } while (p.dist(x, y, w / 2, h / 2) < 150);
      ...
      const drawGame = () => {
        ...
        if (level.type === "openWorld") drawOpenWorldGround(level);
        else if (level.type === "twisty") drawTwistyTrackGround(level);
        else drawTrackGround(level);
        ...
        if (car.isDrifting()) score += 1;

        obstacles.forEach((obs) => {
          if (car.checkCollision(obs)) crashDetected = true;
        });
      ...
      const drawOpenWorldGround = (level: LevelDef) => {
        ...
        p.stroke(50);
        p.strokeWeight(1);
        for (let x = 0; x < level.width; x += 100) p.line(x, 0, x, level.height);
        for (let y = 0; y < level.height; y += 100) p.line(0, y, level.width, y);
        p.noStroke();
        p.fill(45);
        for (let i = 0; i < 15; i++) {
      ...
      const drawTwistyTrackGround = (level: LevelDef) => {
        ...
        p.noFill();
        p.beginShape();
        twistyTrackPoints.forEach((pt) => p.vertex(pt.x, pt.y));
        p.endShape(p.CLOSE);
      ...
      const drawHUD = () => {
        ...
        btn(CW - 80, CH - 50, 70, 35, "BACK", () => { gameState = GAME_STATE.MENU; saveToCloud(); });
      };
      ...
      const drawPauseOverlay = () => {
        ...
        btn(CW / 2 - 100, CH / 2 + 75, 200, 50, "MENU", () => { gameState = GAME_STATE.MENU; saveToCloud(); });
      };
      ...
      const smallBtn = (x: number, y: number, w: number, h: number, label: string, action: () => void) => {
        const hover = p.mouseX > x && p.mouseX < x + w && p.mouseY > y && p.mouseY < y + h;
        p.fill(hover ? p.color(180, 60, 60) : p.color(120, 40, 40));
        p.noStroke();
        p.rect(x, y, w, h, 4);
        p.fill(255);
        p.textAlign(p.CENTER, p.CENTER);
